...
## Example 1: "How do I write a fast CUDA GEMM kernel?"
...
**Command**:
```bash
python3 scripts/query.py "fast gemm kernel" --type kernel --tag gemm --architecture sm100
```

---

## Example 2: "My CUDA kernel has low SM utilization"
...
**Command**:
```bash
python3 scripts/query.py "low sm utilization" --symptom low-sm-utilization
...
**Command**:
```bash
python3 scripts/query.py "tcgen05 in cutlass" --tag tcgen05 --repo cutlass --limit 30
...
## Example 6: "How do I compare CUDA C++, CuTe DSL, and Triton for a kernel?"

**Navigation path**:
1. `queries/by-language.md` → compare language pages
2. Read `wiki/languages/cuda-cpp.md`, `wiki/languages/cute-dsl.md`, and `wiki/languages/triton-blackwell.md`
3. Use `queries/by-kernel-type.md` to see which kernels have examples in each language
4. Follow `sources:` from the language pages into concrete PRs or docs

**Command**:
```bash
python3 scripts/query.py "cuda cpp vs triton vs cute dsl" --type language
python3 scripts/get_page.py lang-cuda-cpp
python3 scripts/get_page.py lang-triton
```
...
## Example 8: "What are the memory-bound kernel optimization tricks?"
...
**Command**:
```bash
python3 scripts/query.py "memory bound kernel" --symptom memory-bound
```
...
**Command**:
```bash
python3 scripts/query.py "flashinfer fp8 moe" --repo flashinfer --tag moe --limit 30
python3 scripts/query.py "flashinfer fp8" --repo flashinfer --tag fp8 --limit 30
...
- Don't recommend DeepEP/DualPipe/EPLB — they're outside this kernel-focused skill.
