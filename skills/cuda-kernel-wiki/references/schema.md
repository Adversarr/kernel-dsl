...
| source-contest | `contest-*` | Archived event/problem pages retained as source material |
...
- The schema supports archived source material in addition to synthesized wiki pages.
- The corpus is strongest on NVIDIA Blackwell and Hopper kernels, but the schema is organized around reusable kernel concepts: techniques, hardware features, kernel types, symptoms, and languages.
- Event-derived pages may remain in `sources/`, but the user-facing interface should favor durable technical framing over event narration.
...
Pages including `sm90` in `architectures` without any `sm100*` variant still require a `blackwell_relevance:` field. This is a validation rule for the current corpus shape, not a statement that the whole skill is only for one architecture family.
