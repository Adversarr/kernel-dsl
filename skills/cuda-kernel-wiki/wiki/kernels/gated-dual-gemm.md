...
Archived benchmark material for this pattern came from the GPU Mode NVFP4 corpus, but the reusable value here is the fused gate-up structure itself: one input tile, two GEMMs, a fused activation epilogue, and no intermediate global-memory roundtrip.
