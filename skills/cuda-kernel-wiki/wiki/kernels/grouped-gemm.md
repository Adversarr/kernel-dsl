...
Archived benchmark material for grouped GEMM came from the GPU Mode NVFP4 corpus, but the durable lesson is broader: variable-M expert workloads live or die by tile scheduling, work distribution, and layout choice.
...
Some archived benchmark results around grouped GEMM were later recognized as harness-sensitive rather than purely kernel-driven. Treat the reported latency as context, but rely on the scheduling and layout lessons on this page rather than on any single leaderboard number.
