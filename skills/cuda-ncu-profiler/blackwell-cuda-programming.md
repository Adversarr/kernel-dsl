...
> This document remains CUDA/Blackwell backend guidance for the generated CUDA kernel after `cuda-ncu-profiler` has identified a bottleneck. The main operational workflow of the skill is now Python-DSL-first, but the optimization advice here still applies to the emitted CUDA running on NVIDIA Blackwell.
